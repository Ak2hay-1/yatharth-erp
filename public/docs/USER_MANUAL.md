...
6. Optional: upload SOPs under **Quality → SOPs**.
...
| **Masters** | Items, Parties, Recipes | Master lists (setup data) |
...
1. **Quality → Complaints →** New — customer, SKU, lot, issue type, description.
...
- **Quality → SOPs** — upload standard operating procedures (PDF/images/Office files, up to 25 MB).
...
| **Recipes** | Admin+ | Finished SKU + ingredient lines + output qty; use scale preview for planned batches |
...
2. **Settings → Users** — create Admin / Staff / Super Admin accounts.
3. **Backup & restore** — download a copy of the SQLite database (`.db`), or enable **automatic backup** to a folder you choose (USB / cloud), with a time of day and how often (hours, days, or weeks). Restore only when you intend to replace current data.
