...
function itemData(formData: FormData) {
  return {
    ...
    unitsPerPkt: Math.max(1, Math.round(parseNum(formData.get("unitsPerPkt"), 1))),
...
export async function createItem(formData: FormData) {
  ...
  if (item.type === "FINISHED") enqueueProductSync(item.sku);
...
export async function updateItem(id: string, formData: FormData) {
  await requireRole(MANAGEMENT);
  const item = await prisma.item.update({ where: { id }, data: itemData(formData) });
  if (item.type === "FINISHED") enqueueProductSync(item.sku);
  revalidatePath("/masters/items");
  redirect("/masters/items");
}
